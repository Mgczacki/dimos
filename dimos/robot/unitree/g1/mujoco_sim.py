...
Self-contained: MuJoCo subprocess + IPC, stream wiring, TF publishing, and
RPC surface in one Module class. No shared base.
"""
...
class G1MujocoConnection(Module):
    """MuJoCo-simulated G1 connection. Publishes sim sensors (lidar, odom,
    color_image, camera_info) which the real WebRTC variant does not expose.
    """
    ...
    def start(self) -> None:
        ...
        self.register_disposable(self._odom_stream().subscribe(self._publish_sim_odom))
        self.register_disposable(self._lidar_stream().subscribe(self.lidar.publish))
        self.register_disposable(self._video_stream().subscribe(self.color_image.publish))
    ...
    def _start_subprocess(self) -> None:
        ...
        while time.time() - start_time < ready_timeout:
            ...
            if self.shm_data.is_ready():
                ...
                def cleanup_on_exit(
                    weak_self: weakref.ReferenceType[G1MujocoConnection] = weak_self,
                ) -> None:
                    instance = weak_self()
    ...
    def _get_odom_message(self) -> SimOdometry | None:
        if self.shm_data is None:
            return None
    ...
    @functools.cache
    def _odom_stream(self) -> Observable[SimOdometry]:
